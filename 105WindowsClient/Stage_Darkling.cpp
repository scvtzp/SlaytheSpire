...
CStage_Darkling::CStage_Darkling()
{
}
...
HRESULT CStage_Darkling::Initialize()
{
	//CSoundMgr::Get_Instance()->PlayBGM(L"BGM_Level1.ogg");

	CObj* pObj = CAbstractFactory<CDarkling>::Create();
	GET_INSTANCE(CObjMgr)->Add_Object(OBJ::OBJ_MONSTER, pObj);

	pObj = CAbstractFactory<CDarkling>::Create();
	pObj->Set_PosPlus(150.f, 30.f);
	GET_INSTANCE(CObjMgr)->Add_Object(OBJ::OBJ_MONSTER, pObj);
...
void CStage_Darkling::Update()
{
	GET_INSTANCE(CObjMgr)->Update();
}

void CStage_Darkling::LateUpdate()
{
